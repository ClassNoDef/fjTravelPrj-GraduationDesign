function initR(type) {
    ...
    $.ajax({
            ...
            success : function(data) {
                ...
                if(data.pageNo>1){
                    pstr+=" <button class='btn btn-sm btn-outline-info' onclick='doSGQuery("+type+","+1+")'>首页</button>&nbsp;"
                }
                if(data.prePage==true){
                    pstr+=" <button class='btn btn-sm btn-outline-info' onclick='doSGQuery("+type+","+(data.pageNo-1)+")'>上一页</button>&nbsp;"
                }
                if(data.nextPage==true){
                    pstr+="<button class='btn btn-sm btn-outline-info' onclick='doSGQuery("+type+","+(data.pageNo+1)+")'>下一页</button>&nbsp;"
                }
                if((data.pageNo!=data.totalPageNum)&&(data.totalRecNum!==0)){
                    pstr+="<button class='btn btn-sm btn-outline-info' onclick='doSGQuery("+type+","+(data.totalPageNum)+")'>末页</button>&nbsp;"
                }
                $("#gspage").html(pstr)
                $("#result").html(str);
                $("#mainTitle").text(data.pageContent[0].guideType.guideTypeName)
                $("Title").text(data.pageContent[0].guideType.guideTypeName+"--列表")
            }
        }
    );
}

function doSGQuery(type,pageNo) {
    var qData={

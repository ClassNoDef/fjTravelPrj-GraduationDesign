function initR(type) {
    ...
    $.ajax({
            ...
            success : function(data) {
                ...
                if(data.pageNo>1){
                    pstr+=" <button class='btn btn-sm btn-outline-info' onclick='doSWQuery("+type+","+1+")'>首页</button>&nbsp;"
                }
                if(data.prePage==true){
                    pstr+=" <button class='btn btn-sm btn-outline-info' onclick='doSWQuery("+type+","+(data.pageNo-1)+")'>上一页</button>&nbsp;"
                }
                if(data.nextPage==true){
                    pstr+="<button class='btn btn-sm btn-outline-info' onclick='doSWQuery("+type+","+(data.pageNo+1)+")'>下一页</button>&nbsp;"
                }
                if((data.pageNo!=data.totalPageNum)&&(data.totalRecNum!==0)){
                    pstr+="<button class='btn btn-sm btn-outline-info' onclick='doSWQuery("+type+","+(data.totalPageNum)+")'>末页</button>&nbsp;"
                }
                $("#wspage").html(pstr)
                $("#result").html(str);
                $("#mainTitle").text(data.pageContent[0].workType.noteTypeName)
                $("Title").text(data.pageContent[0].workType.noteTypeName+"--列表")
            }
        }
    );
}

function doSWQuery(type,pageNo) {
    var qData={
